...
with tab_demo:
    ...
    if st.button("▶ Run Full Research Demo", type="primary"):
        ...
        # =====================================================
        # STAGE 1: Load high-confidence malicious samples
        # =====================================================
        demo_status.info("**Stage 1/3**: Loading high-confidence malicious samples...")
        demo_progress.progress(5)

        try:
            # Load demo samples (generated by run_ablation.py or create on-the-fly)
            demo_path = os.path.join(config.DATA_DIR, "demo_samples.npy")
            if os.path.exists(demo_path):
                X_demo = np.load(demo_path)
            else:
                # Generate on-the-fly
                rf_demo, _, X_test_demo, y_test_demo, _, _, _ = get_eval_data()
                attack_mask = y_test_demo == 1
                X_attacks = X_test_demo[attack_mask]
                probs = rf_demo.predict_proba(X_attacks)[:, 1]
                high_conf = probs >= config.DEMO_MIN_RISK_SCORE
                X_high = X_attacks[high_conf]
                n = min(config.DEMO_SAMPLE_COUNT, len(X_high))
                top_idx = np.argsort(probs[high_conf])[-n:]
                X_demo = X_high[top_idx]
                np.save(demo_path, X_demo)

            rf_demo_model = joblib.load(os.path.join(config.MODEL_DIR, "random_forest.pkl"))
            initial_probs = rf_demo_model.predict_proba(X_demo)[:, 1]

            st.markdown("### 📋 Stage 1: Baseline Detection")
            st.markdown(
                f"Loaded **{len(X_demo)}** handpicked malicious samples where the baseline "
                f"ML model assigns risk score **> {config.DEMO_MIN_RISK_SCORE}**. These represent "
                f"the model's highest-confidence detections."
            )
            ...
            stage1_data = pd.DataFrame({
                ...
                'Status': ['✅ Detected' for _ in initial_probs],
            })
            ...
            col_s1b.metric("All Detected (Risk > 0.5)", f"{sum(initial_probs > 0.5)}/{len(X_demo)}")
            ...
            # =====================================================
            demo_status.info("**Stage 2/3**: Applying FGSM adversarial perturbation (ε=0.05)...")

            from src.training.surrogate import train_surrogate
            train_df_demo = pd.read_csv(os.path.join(config.MODEL_DIR, "train_set.csv"))
            X_train_demo = train_df_demo.drop(columns=['label']).values
            y_train_demo = train_df_demo['label'].values
            clip_vals = (float(X_train_demo.min()), float(X_train_demo.max()))

            surr_demo, _ = train_surrogate(X_train_demo, y_train_demo)

            fgsm_eps = 0.05
            y_demo = np.ones(len(X_demo))
            ...
            try:
                ...
                noise = rng_demo.normal(0, fgsm_eps, X_demo.shape)
                X_perturbed = np.clip(X_demo + noise, X_train_demo.min(axis=0), X_train_demo.max(axis=0))

            perturbed_probs = rf_demo_model.predict_proba(X_perturbed)[:, 1]
            evaded_mask = perturbed_probs < 0.5
            n_evaded = int(evaded_mask.sum())

            st.markdown("### ⚔️ Stage 2: Adversarial Evasion (FGSM ε=0.05)")
            st.markdown(
                f"Applied FGSM perturbation to all {len(X_demo)} samples. "
                f"**{n_evaded}/{len(X_demo)}** samples now evade detection "
                f"(risk score dropped below 0.5)."
            )

            # Before/after table with color-coded deltas
            ...
            col_s2c.metric("Avg Risk Drop", f"{np.mean(perturbed_probs - initial_probs):+.3f}")
            ...
            st.markdown("### 🛡️ Stage 3: Zero-Trust Recovery")
            ...
            if n_evaded > 0:
                ...
                # Build the proof table — most prominent element
                proof_rows = []
                for i in range(len(X_demo)):
                    if evaded_mask[i]:
                        proof_rows.append({
                            ...
                            'ZT Decision': f"{zt_decisions[i].decision} {'⛔' if zt_decisions[i].decision == 'DENY' else '⚠️'}",
                ...
                st.success(
                    "No samples evaded the ML classifier at ε=0.05. "
